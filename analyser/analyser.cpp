...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseCondition() {
		...
		else if (next.value().GetType() == TokenType::MORE_EQUAL) {
			auto err = analyseStatement();
			if (err.has_value())
				return err;
		}
		else if (next.value().GetType() == TokenType::MORE_THAN) {
			auto err = analyseStatement();
			if (err.has_value())
				return err;
		}
		else if (next.value().GetType() == TokenType::LESS_EQUAL) {
			auto err = analyseStatement();
			if (err.has_value())
				return err;
		}
